  ...
  const resolveTicket = async () => {
    try {
      const response = await fetchWithToken(`https://${serverHost}:${serverPort}/update_ticket_status`, {
        ...
        body: JSON.stringify({ ticket_id: ticket.ticket_id, status: 'resolved' }),
      });

      const data = await response.json();

      if (response.ok) {
        Alert.alert(t('success'), t('ticketResolved'));
        setSentReport(!sentReport);
        navigation.goBack();
      } else {
        Alert.alert(t('error'), data.error || t('failedToResolveTicket'));
      }
    } catch (error) {
      Alert.alert(t('error'), error.message);
    }
    ...
    <ImageBackground source={catBackground} style={styles.backgroundImage}>
      <View style={styles.container}>
        ...
        <View style={styles.footer}>
          ...
          <Button mode="contained" onPress={resolveTicket} style={styles.resolveButton}>
            {t('Resolve')}
          </Button>
        </View>
        <Snackbar
...
const styles = StyleSheet.create({
  ...
  resolveButton: {
    marginBottom: 8,
    backgroundColor: '#ff0000', // Add color for the resolve button
  },
  footer: {
    marginTop: 16,
    padding: 16
